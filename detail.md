...
* 종가오팔(매수 없음)
  * (분할 매도2) 전날 종가보다 높을 경우에만 종가로 매도(전날 종가에서 + 0.01 LOC 매도주문) ※ 종가 수동설정한 경우 해당 가격 기준으로 계산 ※제약사항 : 종가를 사용하는 매매법(종가떨사, MOC매도)은 본장에서 실행하면 매매가격 기준은 종가가 아닌 조회시점 현재가 입니다. 종가를 수동으로 입력해서 사용하세요.
