...
* V2.1 : 기본 방법
  ...
  * 쿼터손절 : V2.1과 같으며, 진행률 100%이상(소진시) 한번에 1/4갯수를 무조건 낮은수LOC 매도로 손절후 진행
...
{% tab title="기타" %}
* 분할매도(매수 없음) :LOC로 매도만 하여 분할 손절하며 본장시작여부에 따라 매도가격이 다름
  * (종가LOC 매도) 전일종가 가격으로 0.5회치
  * (낮은수LOC 매도) 전일종가 가격보다 -10% 낮은가격으로 0.5회치

&#x20;  ※제약사항 : 본장에서 실행하는 경우 매도가격 기준은 종가가 아닌 조회시점 현재가 입니다.

* 무지성매수(매도 없음) : 매일 일매수시도금액 만큼 매수만 진행
  * (LOC 매수) \[현재가 + 15%] 1회치
* 종가매도(매수 없음) : 매일 일매수시도금액 만큼 매도만 진행(※손절 가능성 있음)
  * (LOC 매도) \[현재가 - 10%] 1회치
