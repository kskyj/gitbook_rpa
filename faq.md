...
{% hint style="danger" %}
매수금액을 비교해서 작은가격을 하는이유는 하락장에서 매수가 안되는현상을 위한방지 입니다.
{% endhint %}

{% hint style="info" %}
매수개수는 정액법으로, 종목별원금 / 회차(무매법40, TLP30) / 현재가격 의 내림개수로 정합니다. 즉 평단보다 현재가가 낮으면 평소보다 더 매수합니다.
{% endhint %}
...
설정 > 스마트비서 > 보내는 메일 계정을 \[수동] 으로 설정하고, 수동 설정한 지메일 주소로 죠르디를 설정해야 합니다.
{% endhint %}
...
{% hint style="danger" %}
문제 해결하기 > 전원설정 변경 시도하세요.

계속 문제가 발생하면 설정 > 스마트비서 > ㅁ 예수금이 매수금액보자 적을 경우 경고 및 매수 중단 체크해제 하세요.
{% endhint %}
...
암호화된 비밀번호를 복호화해서 입력할때 그 내용을 별도로 개발자에게 전송을 하면 알수 있겠지만 비밀번호를 전송하지 않습니다. 그리고 이건 자동프로그램이 아니더라도 맘만 먹으면 키움증권이던 증권사 프로그램이 아니더라도 모든 인터넷 로그인등 다 만든 개발자가 마음먹으면 비밀번호 수집이 가능하다는 말입니다. 즉 이건 프로그램 신뢰의 문제라고 생각합니다. 궁금하시면 와이어샤크등을 통해 패킷으로 비밀번호를 별도로 다른곳에 전송하는지 확인해보시면 알수 있습니다.
...
{% hint style="warning" %}
영웅문Global은 가상머신에서 간편인증이 안되게 되어 있습니다. 공동인증서만 가능하며, 간편인증을 선택해서 영웅문Global이 종료되는 경우 영웅문을 재설치 하고 공동인증서를 선택하세요.
{% endhint %}
