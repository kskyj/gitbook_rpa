...
* 매매 체결 분석
  * 매매 요청건에 대한 미체결/체결내역 조회를 통한 매매 성공/실패 분석
  * HTS의 미체결/체결내역 알림(카카오톡, 메일, 텔레그램)에 제공
* 하락대비 추가 매수 내역 알림에 표시
  * 하락대비 추가매수 내역 알림(카카오톡, 메일, 텔레그램)에 제공
* 수익률 기준 설정
  * 설정 > 스마트비서 > 수익률 기준
...
* (메리츠)소수점매매 잔고는 가져오지 않도록 처리
* (메리츠)특정환경에서 체결 후 체결내역 팝업 때문에 정지현상 수정
* (메리츠)특정상황에서 잔고 조회 안되는 현상 수정
* 처음 설치시 가이드 안내 설명 개선
* 알림 발송 실패시 프로그램 종료 하지 않도록 수정
* 설정 바꾸고 매매현황판 색상변경되지 않는 현상 수정
* 특정상황에서 완전자동화 시 멈추는 현상 개선
* 매매 실패시 실패, 성공알림 같이 오는 현상 수정
* 검증실패처리 개선


## \[v3.3.5] 2025-07-31

[https://cafe.naver.com/infinitebuying/72913](https://cafe.naver.com/infinitebuying/72913)

#### **기능개선 / 버그수정**

* 매수 또는 매도 한쪽만 성공했을때 실패, 성공 알림 둘다 오는 현상 수정
* 매수 또는 매도 한쪽만 성공했을때 실패 사유 알림에 안나오는 현상 수정
...
* 다계좌/자동화에서 HTS선택 이동 안되는 현상 수정
* HTS 이름, 비밀번호 변경하고 바로 반영안되는 현상 수정
...
* Finviz 종목조회 실패해도 프로그램 종료 되지 않도록 수정
* 메리츠증권 데이터 처리 개선
...
[https://blog.naver.com/fragilememor/223907793035](https://blog.naver.com/fragilememor/223907793035)
...
* 텔레그램 알림 추가
* 인스톨러 변경
* 신규 설치시 kskyj\_RPA에서 설정 가져오기 기능
...
[https://cafe.naver.com/infinitebuying/52481](https://cafe.naver.com/infinitebuying/52481)

#### **새기능**

* 소진후 자동 신규 매수
  * 설정 > 스마트비서 > 소진후 기존 정보로 신규 매수에 원하는 종목 설정 하도록 변경

#### **기능개선 / 버그수정**

* 종목 소진후 신규매매시 종복 중복나오는 현상 수정
* 매매현황판 글씨 틀어진것 조정
...
* 매매현황판
  ...
  * 원금분할 소수점 입력 지원
* 종가떨사
  * 전날 종가보다 낮을 경우에만 종가로 매수(전날 종가에서 - 0.01 LOC 매수주문)
  * 종가 수동설정한 경우 해당 금액 기준으로 계산
  * 매도 없음
* 모든 RPA 설정 일괄 변경
  * 설정 메뉴를 (모든 RPA) 및 개인화로 구역을 나누어 관리
* 주문확인
  * 매매 이상판단 주문확인에 표시
* 영웅문 실행 호환성 향상
  * 설정 > 일반 / 화면 > ㅁHTS 직접실행 체크를 통해 보안프로그램이 실행 못하게 하는경우에도 실행할 수 있는 옵션 추가

#### **기능개선 / 버그수정**

* 매매현황판에 금일매매 제거
* 로그인 순서 명칭 인증서 순서로 변경
* 최초 사용자의 경우 2100 종목 찾지 못하는 현상 수정
* 기본 딜레이 0.2로 변경
* 명칭변경
  * 로그인 순서 > 인증서 순서
  * 종가매도 > MOC매도
* 메일제목에 사용자이름 전부 나오도록 변경
* 쿼터손절 매수/매도 -14% > -10% 로 수정
* 매매안정화 개선

## \[v2.13.1] 2023-05-21

[https://cafe.naver.com/infinitebuying/51612](https://cafe.naver.com/infinitebuying/51612)

#### **기능개선 / 버그수정**

* 무한매수법 V2.2 일때 매수가격 기준가보다 -0.01 적용
...
* 매매현황판
  * 매입금액 및 보유수량 조절 기능
* 카카오톡/메일에 매매 중단(실패)해도 모든정보 표시
  * 실패응답시 에러메시지 + 매매시도내역 + 성공과 동일한 내용 표시
* 매매대상 계산
  * 실제 매매를 해야되는 대상을 미리 계산하고 실제 매매한것과 비교
* 무조건 1개씩 매매하는 기능
  * 설정 > 스마트비서 > 하단에 1개씩 매매
* 무한매수법 V2.2
  * V2.2 목표 수익률에 따라 매도가격 변경
* 쿼터손절법
  * V2.2 최신공식 적용
    * 선택시 기본 10분할
    * 매수 평단 -10% LOC
...
* 안전장치
  ...
  * 무한매수법 V2.2, TLP\_V2.2 일때, 회차(T)가 원금분할 + 1 보다 큰 경우 매도, 분할매도 안하도록 변경
...
* 주문 검증 실패로 매매 중단시 주문 창 무조건 닫기로 변경
...
* 여러계좌중 특정 계좌 선택
* 3배 레버리지 종목이라도 특정 종목만 매매 가능(LABU ㅠㅠ)
